...
$columnIndex = $_POST['order'][0]['column'] ?? 4; // Default to created column
$columnName = $_POST['columns'][$columnIndex]['data'] ?? 'updated_at';
$columnSortOrder = $_POST['order'][0]['dir'] ?? 'desc';

// Validate column name to prevent SQL injection
$allowedColumns = ['firstname', 'lastname', 'email', 'phone', 'updated_at', 'is_active', 'id'];
...
try {
    // Total records
    $stmt = $pdo->query("SELECT COUNT(*) AS allcount FROM agents WHERE is_deleted = 0");
    $records = $stmt->fetch();
    ...
    if ($searchValue != '') {
        $searchQuery = " AND (firstname LIKE :search OR lastname LIKE :search OR phone LIKE :search OR email LIKE :search)";
        $params[':search'] = "%$searchValue%";
    }

    // Total with filter
    $stmt = $pdo->prepare("SELECT COUNT(*) AS allcount FROM agents WHERE is_deleted = 0 $searchQuery");
    $stmt->execute($params);
    $records = $stmt->fetch();
