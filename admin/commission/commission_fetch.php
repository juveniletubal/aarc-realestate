...
try {
    // Total records
    $stmt = $pdo->query("SELECT COUNT(DISTINCT com_ref) AS allcount FROM commissions WHERE is_deleted = 0");
    $records = $stmt->fetch();
    ...
    // Total with filter
    $stmt = $pdo->prepare("SELECT COUNT(DISTINCT com_ref) AS allcount FROM commissions WHERE is_deleted = 0 $searchQuery");
    $stmt->execute($params);
    $records = $stmt->fetch();
    ...
    $sql = "
        SELECT 
            c.id,
            c.assigned_agent AS com_ref,
            GROUP_CONCAT(
                CONCAT(u.firstname, ' ', u.lastname)
                ORDER BY 
                    CASE 
                        WHEN com.role = 'director' THEN 1
                        WHEN com.role = 'manager' THEN 2
                        WHEN com.role = 'downline' THEN 3
                        ELSE 4
                    END
                SEPARATOR ' / '
            ) AS names,
            GROUP_CONCAT(
                CONCAT(com.role, ' (', com.percent, '%)')
                ORDER BY 
                    CASE 
                        WHEN com.role = 'director' THEN 1
                        WHEN com.role = 'manager' THEN 2
        ...
        JOIN clients c ON c.assigned_agent = com.com_ref
