...
try {
    ...
    $stmt = $pdo->query("
        ...
        WHERE u.is_deleted = 0 AND u.role = 'client' AND c.is_deleted = 0
    ");
    $records = $stmt->fetch();
    ...
    foreach ($clientRecords as &$record) {
        ...
        $formattedDate = date('m-d-Y', strtotime($record['first_payment_date']));
        if ($record['first_payment_date'] != NULL) {
            $record['due_date'] = $formattedDate;
        } else {
            $record['due_date'] = '';
        }
